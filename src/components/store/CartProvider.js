...
const cartReducer = (state, action) => {
  ...
  if (action.type === 'ADD') {
    ...
    if (index === -1) {
      ...
      // 이미 추가가 됐던 아이템 -> 수량을 1 올려주면 된다. (모달 안에서만 유효)
      // prevCartItem.amount++; (x) -> 바깥 화면에서는 상품이 꼭 하나씩만 올라가는 것은 아니기 때문
    ...
    */
    ...
    // 업데이트 전의 수량이 1이라면 filter을 이용해서 카트에서 아예 빼버린다.
    // 1보다 크다면 filter로 제거하지 않고 기존 배열에서 수량만 1 내린 채로 업데이트
    let removedItems;
